...
package co.cask.common.security.handlers;

import co.cask.common.logging.AuditLogEntry;
import co.cask.common.security.auth.AccessTokenTransformer;
import co.cask.common.security.auth.TokenState;
...
public class SecurityAuthenticationHttpHandler extends SimpleChannelHandler {
  ...
  private Long findContentLength(ChannelBuffer buffer) {
    ...
    if (index >= 0) {
      // find the following ':'
      int colonIndex = buffer.indexOf(index, bufferEnd, COLON);
